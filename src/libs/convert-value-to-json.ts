...
export function convertValueToJSON(
  value: VariableValue,
  collectionIdToKeyMap: ReturnUniqueKeyIdMaps['idToKey'],
): string | number {
  const isVariableAlias = (v: VariableValue): v is VariableAlias =>
    typeof v === 'object' && 'type' in v && v.type === 'VARIABLE_ALIAS';
  const isRGBorRGBA = (v: VariableValue): v is RGB | RGBA =>
    typeof v === 'object' && 'r' in v && 'g' in v && 'b' in v;
